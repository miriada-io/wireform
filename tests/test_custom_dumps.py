...
@pytest.mark.parametrize(
    ...
    [
        ...
        param(
            datetime.datetime(year=2025, month=1, day=1, hour=12, minute=15, second=0),
            TypeError("datetime.datetime WITHOUT tzinfo is not JSON serializable"),
        ),
        param(
            datetime.datetime.fromtimestamp(1234567890.1),
            TypeError("datetime.datetime WITHOUT tzinfo is not JSON serializable"),
        ),
        param(
            datetime.datetime(year=2025, month=1, day=1, hour=12, minute=15, second=0, tzinfo=datetime.UTC),
